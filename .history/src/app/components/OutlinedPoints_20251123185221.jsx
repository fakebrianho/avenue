...
const OutlinedPointMaterial = shaderMaterial(
  ...
  `
    ...
    void main() {
      ...
      // Core: white circle in the center
      // smoothstep(0.3, 0.0, d) = 1.0 at center (d=0), 0.0 at d=0.3
      float coreRadius = 0.35;
      float core = smoothstep(coreRadius, 0.0, d);
      
      // Outline: pink ring from coreRadius to edge
      // We want pink where core is 0 (outside coreRadius) but inside the point
      float outlineInner = coreRadius;
      float outlineOuter = 0.5;
      float outline = smoothstep(outlineOuter, outlineInner, d) * (1.0 - core);

      // Color: white core, pink outline
      vec3 col = mix(uOutlineColor, uColor, core);
      // Alpha: visible where either core or outline exists
      float alpha = max(core, outline);
