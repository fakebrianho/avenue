...
export function OutlinedPoints() {
  ...
  const handlePointerMove = (e) => {
    ...
    if (e.index !== undefined && e.index !== null) {
      // Get the actual point position from the positions array
      const index = e.index
      const x = positions[index * 3]
      const y = positions[index * 3 + 1]
      const z = positions[index * 3 + 2]
      
      // Transform to world space (accounting for the Points position [0, 28, 0])
      const worldPos = new THREE.Vector3(x, y, z)
      worldPos.add(new THREE.Vector3(0, 28, 0))
      
      setHoverPos(worldPos)
    } else {
      // Fallback to intersection point if index not available
