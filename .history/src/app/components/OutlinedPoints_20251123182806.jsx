...
export function OutlinedPoints() {
  ...
  const handlePointerMove = (e) => {
    ...
    if (e.index !== undefined && e.index !== null && e.index < count) {
      ...
      // Since hover outline is a sibling to main Points, use same local coordinates
      // The main Points has position={[0, 28, 0]}, so we need to account for that
      const localPos = new THREE.Vector3(x, y, z)
      setHoverPos(localPos)
    } else {
      // Fallback: try to convert world point to local space
