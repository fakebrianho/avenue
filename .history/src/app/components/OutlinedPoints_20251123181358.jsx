...
export function OutlinedPoints() {
  ...
  useFrame(() => {
    ...
    if (hoverGeometryRef.current && hoverPos) {
      const positionAttribute = hoverGeometryRef.current.attributes.position
      if (positionAttribute) {
        positionAttribute.setXYZ(0, hoverPos.x, hoverPos.y, hoverPos.z)
    ...
    <>
      ...
      {hoverPos && (
        <Points ref={hoverRef} frustumCulled={false}>
          <bufferGeometry ref={hoverGeometryRef}>
            <bufferAttribute
              attach="attributes-position"
              array={new Float32Array([hoverPos.x, hoverPos.y, hoverPos.z])}
              count={1}
              itemSize={3}
            />
          </bufferGeometry>
