...
export function OutlinedPoints() {
  ...
  const handlePointerMove = (e) => {
    // e.point is already in world coordinates
    // Since hover outline is a sibling, we can use it directly
    if (e.index !== undefined && e.index !== null) {
      // Use the actual point position from the positions array
      const index = e.index
      const x = positions[index * 3]
      const y = positions[index * 3 + 1]
      const z = positions[index * 3 + 2]
      
      // Create local position and transform to world space
      const localPos = new THREE.Vector3(x, y, z)
      
      // Get the world matrix of the Points object
      if (cloudRef.current) {
        cloudRef.current.localToWorld(localPos)
        setHoverPos(localPos)
      } else {
        // Fallback: add the Points position offset manually
        localPos.add(new THREE.Vector3(0, 28, 0))
        setHoverPos(localPos)
      }
    } else {
      // Fallback to intersection point
