...
const OutlinedPointMaterial = shaderMaterial(
  ...
  `
    ...
    void main() {
      ...
      // Core: white circle in center (fades from center to edge)
      float coreRadius = 0.35;
      float core = 1.0 - smoothstep(0.0, coreRadius, d);
      
      // Outline: pink ring near edge
      // Create a ring that's visible between two radii
      float outlineInner = 0.5 - uOutlineWidth;
      float outlineOuter = 0.5;
      float outline = smoothstep(outlineInner, outlineOuter, d) * (1.0 - smoothstep(outlineOuter, outlineOuter + 0.02, d));
      
      // Blend: white core, pink outline
      // Use core to determine if we're in the center (white) or outline area (pink)
