...
const OutlinedPointMaterial = shaderMaterial(
  ...
  `
    ...
    void main() {
      ...
      // Make outline visible in the ring area with smooth edges
      float outline = smoothstep(outlineInner - 0.1, outlineInner, d) * smoothstep(outlineOuter, outlineOuter - 0.1, d);
...
export function OutlinedPoints({ isLoading = false }) {
  ...
  const {
		...
		src: 'hover.mp3', // or a different file
		autoplay: false,
		loop: false,
		volume: 0.1,
	  })
  ...
  useEffect(() => {
		if (data) {
      window.open(data.element.link, '_blank').focus();
  ...
  // Debounced audio play function
  // const playHoverAudio = useCallback(() => {
  //   // Don't play if loader is visible
  //   if (isLoading) return
    
  //   // Don't play if audio is not loaded
  ...
  //   // Debounce the audio play
  //   debounceTimerRef.current = setTimeout(() => {
  //     if (!isAudioPlaying && !isLoading && isAudioLoaded) {
  //       playAudio()
  //     }
  //   }, 100) // 100ms debounce
  // }, [isLoading, isAudioPlaying, isAudioLoaded, playAudio])

  const hoverPositions = useMemo(() => {
    if (!hoverPos) return null
    // playHoverAudio()
    return new Float32Array([hoverPos.x, hoverPos.y, hoverPos.z])
  }, [hoverPos, playHoverAudio])
