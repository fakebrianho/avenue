...
const OutlinedPointMaterial = shaderMaterial(
  ...
  `
    ...
    void main() {
      ...
      // Core: white circle in the center (d from 0 to ~0.3)
      float coreRadius = 0.3;
      float core = 1.0 - smoothstep(0.0, coreRadius, d);
      
      // Outline: pink ring near the edge
      // Create a ring by making it visible between two radii
      float outlineStart = 0.5 - uOutlineWidth;
      float outlineEnd = 0.5;
      float outline = smoothstep(outlineStart, outlineEnd, d) * (1.0 - smoothstep(outlineEnd, outlineEnd + 0.05, d));

      // Combine: if we're in core area, use white; otherwise use pink for outline
      vec3 col = mix(uOutlineColor, uColor, core);
      float alpha = max(core, outline * 0.8);
