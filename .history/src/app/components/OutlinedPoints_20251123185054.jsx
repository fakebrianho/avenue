...
const OutlinedPointMaterial = shaderMaterial(
  ...
  `
    ...
    void main() {
      ...
      // Core: white circle in the center (small radius)
      float coreRadius = 0.3;
      float core = smoothstep(coreRadius, 0.0, d);
      
      // Outline: pink ring around the edge
      float outlineInner = 0.5 - uOutlineWidth;
      float outlineOuter = 0.5;
      float outline = smoothstep(outlineInner, outlineOuter, d) * (1.0 - core);

      // Combine: core is white, outline is pink
      vec3 col = mix(uOutlineColor, uColor, core);
      float alpha = core + outline;
